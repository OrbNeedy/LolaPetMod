...
{
    public class DarknessLolaPetProjectile : ModProjectile
    {
        ...
        private const string bodyRoute = "LolaPet/Content/Lola/DarknessLolaPetProjectile";
        ...
        int lastIndex = (int)DefaultStyles.DarknessTrigger;
        ...
        public override void Load()
        {
            if (!Main.dedServ)
            {
                ...
                // Hologram changes sprite, but defaults to _DarknessTrigger
                Hologram = ModContent.Request<Texture2D>(hologramRoute + "_DarknessTrigger");
                // Hologram Glowmask changes with hologram, but glows
                HologramGlowmask = ModContent.Request<Texture2D>(hologramRoute + "_DarknessTriggerGlowmask");
        ...
        public override void Unload()
        {
            ...
            currentIndex = (int)DefaultStyles.DarknessTrigger;
            lastIndex = (int)DefaultStyles.DarknessTrigger;
        }
        ...
        public override void AI()
        {
            ...
            if (!player.dead && player.active && player.HasBuff<DarknessLolaPetBuff>())
            {
                Projectile.timeLeft = 2;
            }
        ...
        private void HandleIndexChange(Player player)
        {
            ...
            // If there is no key for the item type, set current index to -2
        ...
        private void HandleTransformation(Player player)
        {
            ...
            if (transforming)
            {
                bodyFrame = transformTimer > 75? 0 : 1; //0 + (120 - transformTimer) / 90;
                wingFrame = 0;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                DrawDefaultSphere(Color.White, SpriteEffects.None);
                return false;
            }
            ...
            if (transforming)
            {
                float transparency = ((float)transformTimer / 20f);
            ...
            if (turningBack)
            {
                float transparency = 1 - ((float)transformTimer / 20f);
                DrawColoredSphere(lightColor * transparency * 0.7f, flipping, transparency * 0.7f);
